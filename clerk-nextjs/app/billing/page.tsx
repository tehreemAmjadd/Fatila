...
import {
  CreditCard, Zap, Lock, CheckSquare, Clock, AlertTriangle,
  LayoutDashboard, Bot, Search, Bookmark, BarChart2,
  Mail, Phone, Upload, Megaphone, Menu, X,
} from "lucide-react";
...
function BillingContent() {
  ...
  useEffect(() => {
    ...
    const nodes: any[] = Array.from({length:80}, () => ({
  ...
  const isPaid        = dbUser?.subscriptionStatus === "active";
      ...
      <div id="sidebar" className={`sidebar ${sidebarActive?"active":""}`}>
        ...
        <nav>
          {NAV.map(({href,label,Icon})=>(
            <a key={href} href={href} className={href==="/billing"?"active":""}>
