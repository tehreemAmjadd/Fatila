...
import {
  ...
  Menu, X, PhoneCall, PhoneOff, PhoneMissed, PhoneIncoming,
  MessageSquare, ClipboardList, Lock, AlertTriangle,
  TrendingUp, Users, ChevronDown, Send, ExternalLink,
} from "lucide-react";
...
export default function CallsPage() {
  ...
  useEffect(() => {
    ...
    const nodes: any[] = Array.from({length:80},()=>({x:Math.random()*canvas.width,y:Math.random()*canvas.height,vx:(Math.random()-.5)*.8,vy:(Math.random()-.5)*.8}));
    let animId: number;
    const draw = () => {
      ...
      <div id="sidebar" className={`sidebar ${sidebarActive?"active":""}`}>
        ...
        <nav>
          {NAV.map(({href,label,Icon})=>(
            <a key={href} href={href} className={href==="/calls"?"active":""}>
