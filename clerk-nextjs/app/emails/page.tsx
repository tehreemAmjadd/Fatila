...
import {
  ...
  Menu, X, Send, Sparkles, Settings, Lock, AlertTriangle,
...
export default function EmailsPage() {
  ...
  useEffect(() => {
    ...
    const nodes: any[] = Array.from({length:70},()=>({x:Math.random()*canvas.width,y:Math.random()*canvas.height,vx:(Math.random()-.5)*.7,vy:(Math.random()-.5)*.7}));
    const draw = () => {
      ctx.clearRect(0,0,canvas.width,canvas.height);
      nodes.forEach(n=>{n.x+=n.vx;n.y+=n.vy;if(n.x<0||n.x>canvas.width)n.vx*=-1;if(n.y<0||n.y>canvas.height)n.vy*=-1;ctx.beginPath();ctx.arc(n.x,n.y,1.8,0,Math.PI*2);ctx.fillStyle="#00ff99";ctx.fill();});
      for(let i=0;i<nodes.length;i++) for(let j=i+1;j<nodes.length;j++){const dx=nodes[i].x-nodes[j].x,dy=nodes[i].y-nodes[j].y,d=Math.sqrt(dx*dx+dy*dy);if(d<120){ctx.beginPath();ctx.moveTo(nodes[i].x,nodes[i].y);ctx.lineTo(nodes[j].x,nodes[j].y);ctx.strokeStyle=`rgba(0,255,153,${.12*(1-d/120)})`;ctx.stroke();}}
  ...
  const canEmail      = planCfg.canEmail;
    ...
    <>
      ...
      <div id="sidebar" className={`sidebar ${sidebarActive?"active":""}`}>
        ...
        <nav>
          {NAV.map(({href,label,Icon})=>(
            <a key={href} href={href} className={href==="/emails"?"active":""}>
        ...
        <div className="sb-footer">
          ...
          {!canEmail&&<a href="/billing" className="sb-upgrade">Upgrade</a>}
        </div>
      </div>
      ...
      <style jsx>{`
        *{margin:0;padding:0;box-sizing:border-box;font-family:'Inter',sans-serif;}
        ...
        /* Compose */
        .compose-layout{max-width:680px;}
        ...
        @media(max-width:1100px){.bulk-layout{grid-template-columns:1fr;}.ai-grid{grid-template-columns:1fr;}}
